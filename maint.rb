...
def match_concurrences_to_db(date='1975')
	#NOTE: there are a couple anomalies -- xml has a few more cits than text fore some reason...
  ...
  DB[:ALL].aggregate(pipeline).each do |opinion|
    ...
    #matches.reject!{|item| item[:regexp_match_text].match /\([^()]{0,100}concur[^()]{0,100}\)/}
  ...
  #DB[:concur_matches].delete_many {}
  #DB[:concur_matches].insert_many matches
end
